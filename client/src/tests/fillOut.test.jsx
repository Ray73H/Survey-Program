...
// 👇 All vi.mocks must appear before runtime logic
...
describe("FillSurvey", () => {
  ...
  const input = screen.getByRole("textbox"); // assumes there's only one input on this screen
  fireEvent.change(input, { target: { value: "TEST" } });

  // Check the typed value is reflected
...
it("should navigate when icon-only back button is clicked and confirmed", async () => {
  ...
  const backButton = buttons[0]; // 👈 assuming it's the first
