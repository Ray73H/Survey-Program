...
export const calculateSurveyMetrics = async (req, res) => {
  try {
	...
	const completionRate = startedAnswers.length
	? (completedAnswers.length / startedAnswers.length) * 100
	: 0;
    ...
    const detailedAnswers = await Answer.find({ completed: true }).populate({
      path: 'surveyId',
      select: 'questions',
    }).lean();

    let mcTimes = [], textTimes = [];
...
export const getAnswerMetricsPerSurvey = async (req, res) => {
	try {
		...
		const completionRate = data.started
